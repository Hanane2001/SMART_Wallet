...
$std = $conn->prepare("SELECT * FROM cards WHERE idCard = ? AND idUser = ?");
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $std = $conn->prepare("UPDATE cards SET cardName = ?, bankName = ?, cardNumber = ?, currentBalance = ?, isMain = ? WHERE idCard = ? AND idUser = ?");
    $std->bind_param("sssdiii", $cardName, $bankName, $cardNumber, $currentBalance, $isMain, $cardId, $userId);
    if($std->execute()){
