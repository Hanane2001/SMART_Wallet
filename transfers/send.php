...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $std->bind_param("ii", $sender_card_id, $userId);
    $std->execute();
    ...
    $mainC = $conn->prepare("SELECT idCard FROM cards WHERE idUser = ? AND isMain = 1 LIMIT 1");
    $mainC->bind_param("i", $receiver_id);
    $mainC->execute();
    $mainCRes = $mainC->get_result();
    
    if ($mainCRes->num_rows === 0) {
        header("Location: list.php?error=receiver_no_main_card");
        exit();
    }
    
    $receiver_card = $mainCRes->fetch_assoc();
    ...
    try {
        $str = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'completed')
        ");
        $str->bind_param("iidiis", $userId, $receiver_id, $amount, $sender_card_id, $receiver_card_id, $description);
        $str->execute();
